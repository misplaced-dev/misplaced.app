...
const ProfileContainer = () => {
  ...
  const fetchProfile = async (userId) => {
    try {
      ...
      const posts = await PostService.getPostsByUserId(userId).then(
        async (res) => {
          return res.data;
        }
      );
      for (let i = 0; i < posts.length; i++) {
        const media = await MediaService.getMediaByPostId(posts[i]._id).then(
          (res) => {
  ...
  return (
    ...
    >
      <Image
        ...
        style={{ alignSelf: "center", width: 100, height: 100, marginTop: 30 }}
      />
      <Text style={{ textAlign: "center", fontSize: 25, paddingTop: 15 }}>
        Welcome Back {name}!
      </Text>
      <Text
        style={{
      ...
      <View style={styles.container}>
        {posts.map((post) => (
          <Postcard
            ...
            onPress={() =>
              navigation.navigate("Post Page | Misplaced", { key: post._id })
            }
          />
        ))}
      </View>
